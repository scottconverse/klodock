#!/bin/bash
# KloDock v1.1 — Full QA Test Suite
cd "$(dirname "$0")/.."
...
echo "╔══════════════════════════════════════════════════════╗"
echo "║       KloDock v1.1 — Full QA Test Suite             ║"
echo "╚══════════════════════════════════════════════════════╝"
...
[ "$PKG_VER" = "1.1.0" ] && ok "Version is 1.1.0" || fail "Version is $PKG_VER"
...
grep -q "1.1" README.md && ok "README v1.1" || fail "README outdated"
grep -q "1.1" README-full.md && ok "README-full v1.1" || fail "README-full outdated"
grep -q "v1.1" website/index.html && ok "Landing page v1.1" || fail "Landing outdated"
...
INST="src-tauri/target/release/bundle/nsis/KloDock_1.1.0_x64-setup.exe"
test -f "$INST" && ok "Installer exists" || fail "Installer missing"
echo "$INST" | grep -q "1.1.0" && ok "Installer versioned correctly" || fail "Installer wrong version"
